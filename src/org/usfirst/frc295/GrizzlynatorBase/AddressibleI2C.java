 ...
 * 
 * For the RoboRio, the I2C library changed to 7 bit addressing
 * so the range is 1 to 128 instead of 2 to 254. 
 * The default address is 112 to the RoboRio.
 * 
 * The Arduino library uses 8 bit addressing.
 * For example I used the Arduino example code to set the address to 4 
 * and then accessed the sensor on address 2 from the RoboRio. 
...
public class AddressibleI2C extends SensorBase
{
   ...
   * @param deviceAddress The base address of the ultrasonic device on the I2C bus.
   */
   ...
   */
  public synchronized boolean transaction(byte deviceAddress, 
		                                  byte[] dataToSend, int sendSize,
    ...
    status = I2CJNI.i2CTransaction((byte) m_port.value, deviceAddress, dataToSendBuffer,
   ...
   */
  public synchronized boolean transaction(byte deviceAddress, 
		                                  ByteBuffer dataToSend, int sendSize,
    ...
    return I2CJNI.i2CTransaction((byte) m_port.value, deviceAddress, dataToSend,
  ...
  public boolean readOnly(byte deviceAddress, byte[] buffer, int count) {
    ...
    int retVal = I2CJNI.i2CRead((byte) m_port.value, deviceAddress, dataReceivedBuffer,
                                (byte) count);
  ...
  public boolean readOnly(byte deviceAddress, ByteBuffer buffer, int count) {
    ...
    return I2CJNI.i2CRead((byte) m_port.value, deviceAddress, buffer, (byte) count)
        < count;
  }
   ...
   */
  public boolean verifySensor(byte deviceAddress, int registerAddress, int count,
                              byte[] expected) {

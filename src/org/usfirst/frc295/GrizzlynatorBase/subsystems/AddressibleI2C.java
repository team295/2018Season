 ...
 *
 * For the RoboRio, the I2C library changed to 7 bit addressing so the range is
 * 1 to 128 instead of 2 to 254. The default address is 112 to the RoboRio.
 *
 * The Arduino library uses 8 bit addressing. For example I used the Arduino
 * example code to set the address to 4 and then accessed the sensor on address
 * 2 from the RoboRio. The clock and data do need external 120 ohm pullups also.
 */
public class AddressibleI2C extends SensorBase
{
	 ...
	 * @param deviceAddress
	 *            The base address of the ultrasonic device on the I2C bus.
	 */
	public AddressibleI2C(Port port, int deviceAddress)
	{
		m_port = port;
	 ...
	 */
	public synchronized boolean transaction(byte deviceAddress, byte[] dataToSend, int sendSize, byte[] dataReceived,
			int receiveSize)
	{
		final int status;
		...
		status = I2CJNI.i2CTransaction((byte) m_port.value, deviceAddress, dataToSendBuffer, (byte) sendSize,
	 ...
	 * @return Transfer Aborted... false for success, true for aborted.
	 */
	public synchronized boolean transaction(byte deviceAddress, ByteBuffer dataToSend, int sendSize,
			ByteBuffer dataReceived, int receiveSize)
	{
		...
		return I2CJNI.i2CTransaction((byte) m_port.value, deviceAddress, dataToSend, (byte) sendSize, dataReceived,
				(byte) receiveSize) < receiveSize;
	}
	 ...
	 * @return Transfer Aborted... false for success, true for aborted.
	 */
	public boolean addressOnly(byte deviceAddress)
	{
		return transaction(deviceAddress, (byte[]) null, (byte) 0, null, (byte) 0);
	}
	 ...
	 */
	public synchronized boolean write(byte deviceAddress, int registerAddress, int data)
	{
		...
		return I2CJNI.i2CWrite((byte) m_port.value, deviceAddress, dataToSendBuffer,
	 ...
	 */
	public synchronized boolean writeBulk(byte deviceAddress, byte[] data)
	{
		ByteBuffer dataToSendBuffer = ByteBuffer.allocateDirect(data.length);
		dataToSendBuffer.put(data);

		return I2CJNI.i2CWrite((byte) m_port.value, deviceAddress, dataToSendBuffer, (byte) data.length) < data.length;
	}
	 ...
	 */
	public synchronized boolean writeBulk(byte deviceAddress, ByteBuffer data, int size)
	{
		if (!data.isDirect())
		{
		...
		return I2CJNI.i2CWrite((byte) m_port.value, deviceAddress, data, (byte) size) < size;
	}
	 ...
	 * @return Transfer Aborted... false for success, true for aborted.
	 */
	public boolean read(byte deviceAddress, int registerAddress, int count, byte[] buffer)
	{
		if (count < 1)
		{
		...
		return transaction(deviceAddress, registerAddressArray, registerAddressArray.length, buffer, count);
	}
	 ...
	 * @return Transfer Aborted... false for success, true for aborted.
	 */
	public boolean read(byte deviceAddress, int registerAddress, int count, ByteBuffer buffer)
	{
		if (count < 1)
		{
			throw new BoundaryException("Value must be at least 1, " + count + " given");
		...
		return transaction(deviceAddress, dataToSendBuffer, 1, buffer, count);
	}
	 ...
	 * @return Transfer Aborted... false for success, true for aborted.
	 */
	public boolean readOnly(byte deviceAddress, byte[] buffer, int count)
	{
		if (count < 1)
		{
			throw new BoundaryException("Value must be at least 1, " + count + " given");
		...
		int retVal = I2CJNI.i2CRead((byte) m_port.value, deviceAddress, dataReceivedBuffer, (byte) count);
	 ...
	 * @return Transfer Aborted... false for success, true for aborted.
	 */
	public boolean readOnly(byte deviceAddress, ByteBuffer buffer, int count)
	{
		if (count < 1)
		{
			throw new BoundaryException("Value must be at least 1, " + count + " given");
		...
		return I2CJNI.i2CRead((byte) m_port.value, deviceAddress, buffer, (byte) count) < count;
	}
	 ...
	 *      auto-increment.
	 */
	public boolean verifySensor(byte deviceAddress, int registerAddress, int count, byte[] expected)
	{
		...
		for (int i = 0, curRegisterAddress = registerAddress; i < count; i += 4, curRegisterAddress += 4)
		{
			...
			if (transaction(deviceAddress, dataToSendBuffer, 1, deviceData, toRead))
			{
				return false;
			}
